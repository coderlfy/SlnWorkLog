...
namespace BusinessBase
{
    public class GeneralAccessor
    {
        ...
        public int GetCustomBusiness(string sql,
        ...
        /// <returns>符合where条件的记录条数（去除分页）</returns>
        private int getCustomDs(DataSet fillDs,
        ...
        /// <param name="conditions">查询条件</param>
        /// <returns>符合where条件的记录条数（去除分页）</returns>
        public int GetSingleTBySP(DataSet fillDs,
            DBConditions conditions)
        {
            #region
            return getCustomDs(fillDs, conditions,
        ...
        private void gettablekey(DataSet destDs,
        ...
        public int GetMaxAddOne(DataSet destDs)
        {
            ...
            try
            {
                string tablename = "";//(destDs.Tables.Count > 0) ? destDs.Tables[0].TableName : "tablename";
                string keyname = "";
